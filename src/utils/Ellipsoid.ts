...
export class Ellipsoid {
  static readonly WGS84 = /*#__PURE__*/ new Ellipsoid(6378137, 6378137, 6356752.3142451793);
  ...
  projectOnSurface(
    ...
    return projectOnEllipsoidSurface(position, this.reciprocalRadiiSquared(), result, options);
  }
  ...
  getOsculatingSphereCenter(
    ...
    const normal = vectorScratch1
      .set(surfacePosition.x / a2, surfacePosition.y / a2, surfacePosition.z / b2)
      .normalize();
    return result.copy(normal.multiplyScalar(-radius).add(surfacePosition));
  }

  getNormalAtHorizon(position: Vector3, direction: Vector3, result = new Vector3()): Vector3 {
    invariant(this.radii.x === this.radii.y);
    const a2 = this.radii.x ** 2;
    const b2 = this.radii.z ** 2;
    const p = position;
