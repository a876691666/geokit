...
export class Geodetic {
  ...
  setFromECEF(
    ...
    const reciprocalRadiiSquared = ellipsoid.reciprocalRadiiSquared(vectorScratch1);
    ...
    if (projection == null) {
      throw new Error(`Could not project position to ellipsoid surface: ${position.toArray()}`);
    }
    const normal = vectorScratch1.multiplyVectors(projection, reciprocalRadiiSquared).normalize();
  ...
  toECEF(
    ...
    const ellipsoid = options?.ellipsoid ?? Ellipsoid.WGS84;
    const radiiSquared = vectorScratch1.multiplyVectors(ellipsoid.radii, ellipsoid.radii);
